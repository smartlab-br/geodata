...
class GeoImporter:
    ...
    def update_progress(self, step):
        ...
        print(
            f"Downloading and unzipping: {self.total_done}/{self.total_files} [{int(self.total_done / self.total_files * 100)}%]  ",
            end="\r",
            flush=True
        )
    ...
    def download_file(self, key, value, resolution, id_au, skip_existing):
        ...
        # Formato de renderização da malha. Útil quando o usuário preferir informar o formato de renderização
        # diretamente na URL do navegador, sem a necessidade de informar o parâmetro Accept.
        fmt = 'application/vnd.geo+json'

        for key_detail, value_detail in enumerate(resolution):
            for quality in qualities:
                f_name = self.select_destination(value, value_detail, quality, id_au)
    ...
    def download_and_unzip(self, ftp_dir, dirname, zip_file_name, dest, unit=None):
    ...
    def run(self):
        ...
        with multiprocess.Pool(processes=8) as pool:
            pool.starmap(self.download_and_unzip, pool_args)

        # Download and unzip topologies from IBGE FTP service (municipalities, micro-regions, meso-regions states and
        # regions).
        ftp = FTPHost.connect("geoftp.ibge.gov.br", user="anonymous", password="anonymous@")
